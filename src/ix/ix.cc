...
namespace PeterDB {
    ...
    RC IX_ScanIterator::getLeafHeaderFromPage(char* pageData, char &nodeType, short &nodeSize, int &nodeParentPageNum, PageNum &rightPageNum, PageNum &overflowPageNum) {
        ...
        memcpy(&nodeParentPageNum, pageData + offset, sizeof(int));
        offset += sizeof(int);
    ...
    RC IX_ScanIterator::getLeafEntryFromPage(char *pageData, PageNum &rightPageNum, short &entryCount, RID &rid, void *key) {
        ...
        char nodeType;
        short nodeSize;
        int nodeParentPageNum;
        PageNum overflowPageNum;
        if (getLeafHeaderFromPage(pageData, nodeType, nodeSize, nodeParentPageNum, rightPageNum, overflowPageNum) == -1) { return -1; }
    ...
    RC BTree::createParentForSplitLeafNode(IXFileHandle &ixFileHandle, Node *&targetNode, LeafNode *&newNode) {
        ...
        //Debug
        int keySize = newNode->leafEntries[0].keySize;
    ...
    BTree::updateParentOfLeaf(IXFileHandle &ixFileHandle, Node *&targetNode, LeafNode *&newNode, InternalNode *&parent) {
        ...
        int copyKeySize = newNode->leafEntries[0].keySize;
    ...
    RC BTree::splitInternalNode(IXFileHandle &ixFileHandle, InternalNode *&targetNode, InternalNode *&newInternalNode) {
        ...
        for (int i = internalEntryNum / 2 + 1; i < internalEntryNum; i++) {
            ...
            if (attrType == TypeVarChar) {
                int keySize;
                memcpy(&keySize, moveEntry.key, sizeof(int));
            }
    ...
    RC BTree::insertEntry(IXFileHandle &ixFileHandle, const LeafEntry &pair) {
        ...
        if (root == nullptr) {
            ...
            if (dynamic_cast<LeafNode *>(targetNode)->sizeInPage <= PAGE_SIZE) {
                ...
                LeafNode *newNode;
                if (splitLeafNode(ixFileHandle, reinterpret_cast<LeafNode *&>(targetNode), newNode) == -1) { return -1; }
